...
const KitchenOrders = () => {
  ...
  const { kitchenOrders, search, selectedOrderId, setSearch, selectOrder } =
    useKitchenState(kitchenState);

  const filteredOrders = kitchenOrders.filter(
    (order) =>
      order.id.toLowerCase().includes(search.toLowerCase()) ||
      order.restaurantId.toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <div className="">
      <div className=" md:flex justify-between items-center mb-5">
        <h1 className="text-2xl font-semibold pb-1">Kitchen Orders</h1>
      ...
      <div className="flex flex-col gap-3">
        ...
        {filteredOrders.length === 0 && (
          <p className="text-gray-500">No kitchen orders found.</p>
        )}
      </div>
    </div>
  );
