...
interface EventModalProps {
  ...
  year: number;
  onClose: () => void;
  onFormChange: (field: keyof EventForm, value: string | number) => void;
...
const EventModal = ({
  ...
  const isEditing = modal.editId != null;

  const isHoliday = modal.mode === "holiday";
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <input
              ...
              value={form.date}
              onChange={(e) => onFormChange("date", Number(e.target.value))}
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300"
            />
          </div>
          ...
          {!isHoliday && (
            <div>
              ...
              <select
                value={form.type}
                onChange={(e) =>
                  onFormChange("type", e.target.value as EventType)
                }
                className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300"
              >
